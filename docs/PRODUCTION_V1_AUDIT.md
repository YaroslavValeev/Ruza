...
| Вернуть полный test gate | PASS | GitHub checks `api-tests`, `dashboard-build`, `production-env-guard-linux`, `production-env-guard-windows` green on PR #4; local pytest/build commands are documented | Re-run after each commit |
