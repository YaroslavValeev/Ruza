...
Current release candidate: `v1.0.0-rc.14`
...
| Получить release candidate SHA | PASS | Tag `v1.0.0-rc.14` must point at the same HEAD verified by `scripts/production-v1-local-audit.ps1` | Create final tag after merge |
| Вернуть полный test gate | PASS | GitHub checks `api-tests`, `dashboard-build`, `production-env-guard-linux`, `production-env-guard-windows`, `clean-release-tree-guard-linux`, `clean-release-tree-guard-windows`, `staging-proof-guard-linux`, `staging-proof-guard-windows`, `server-healthcheck-guard-linux`, `restore-backup-guard-linux`, `restore-backup-guard-windows` are required on PR #4; local pytest/build/audit commands are documented | Re-run after each commit |
